...
def test_configuration_driven_package_creates_one_attempt_smoke_run(db_session) -> None:
    ...
    command_timeouts = sum(
        ...
        for command in db_session.get(BenchmarkRecord, "looper.fixture.config-driven@1.1.0")
        .manifest_json["spec"]["runtime"]["commands"]
        .values()
    )
    assert experiment.spec_json["budget"]["wall_time_seconds"] >= command_timeouts + 300
