...
def test_single_node_contract_requires_one_fixed_machine() -> None:
    ...
    dedicated_load_generator = deepcopy(manifest)
    dedicated_load_generator["spec"]["infrastructure"]["primaryNodeGroup"] = "target"
    dedicated_load_generator["spec"]["infrastructure"]["nodeGroups"].append({
        "id": "load-generator",
        "role": "load-generator",
        "count": {"minimum": 1, "default": 1, "maximum": 1},
        "includedInScore": False,
    })
    assert single_node_contract(dedicated_load_generator)["id"] == "target"
