...
export function ExperimentsPage() {
  ...
  return <div className="page"><PageHeader title="选型研究" description="管理采购问题、候选资源和场景证据。" actions={<Link className="button primary" to="/experiments/new"><Plus size={16}/>新建选型研究</Link>} />
    ...
    {query.isLoading ? <LoadingState /> : query.isError ? <ErrorState error={query.error} onRetry={() => query.refetch()} /> : items.length ? <section className="panel table-panel"><ExperimentTable experiments={items}/></section> : <EmptyState title="没有匹配的研究" description={search || status !== 'all' ? '请调整搜索词或状态筛选。' : '先创建一项场景化选型研究。'} action={!search && status === 'all' ? <Link className="button primary" to="/experiments/new">新建选型研究</Link> : undefined} />}
  </div>;
}
