...
import { ArrowRight, CheckCircle2, ClipboardList, Copy, Download, ExternalLink, History, KeyRound, RefreshCw, ShieldCheck, Trash2, TriangleAlert } from 'lucide-react';
...
import type { CloudOrder, CloudOrderEvent, Target } from '../lib/types';
...
const orderQueryRecovery = {
  retry: (failureCount: number, error: unknown) => {
    if (failureCount >= 3) return false;
    if (error instanceof ApiError && error.status >= 400 && error.status < 500 && ![408, 429].includes(error.status)) return false;
    return true;
  },
  retryDelay: (attemptIndex: number) => Math.min(250 * 2 ** attemptIndex, 2_000),
...
function OrderList() {
  ...
  const query = useQuery({
    queryKey: ['cloud-orders', status],
    queryFn: () => api.orders(status),
    refetchInterval: current => current.state.data ? 15_000 : 1_000,
    ...orderQueryRecovery,
  });
  const deletion = useMutation({
    mutationFn: (id: string) => api.deleteOrder(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cloud-orders'] }),
  });
  const remove = (order: CloudOrder) => {
    if (!['awaiting_confirmation', 'failed', 'expired'].includes(order.status)) return;
    if (window.confirm(`确定删除云订单“${order.spec.instanceName}”吗？订单记录和关联审计事件将被移除，此操作不可撤销。`)) deletion.mutate(order.id);
  };
  return <div className="page"><PageHeader title="云订单" description="报价快照、购买记录、供应商响应和纳管资源的审计轨迹。" actions={<button className="button secondary" type="button" disabled={query.isFetching} onClick={() => query.refetch()}><RefreshCw className={query.isFetching ? 'spin' : undefined} size={15} />{query.isFetching ? '刷新中' : '刷新'}</button>} /><div className="toolbar"><label className="select-field"><ClipboardList size={15} /><select aria-label="订单状态" value={status} onChange={event => setStatus(event.target.value)}><option value="">全部状态</option>{Object.entries(labels).map(([key, value]) => <option key={key} value={key}>{value}</option>)}</select></label><span className="result-count">{query.data?.items.length || 0} 个订单</span></div>{query.isError && query.data && <div className="error-banner" role="status"><TriangleAlert size={18} /><span>订单列表刷新失败，当前显示最近一次数据。</span><button className="button secondary" type="button" onClick={() => query.refetch()}>重试</button></div>}{!query.data ? <LoadingState /> : query.data.items.length ? <section className="panel table-panel"><div className="table-wrap"><table><thead><tr><th>订单</th><th>云厂商</th><th>资源</th><th>小时金额</th><th>状态</th><th>更新时间</th><th /></tr></thead><tbody>{query.data.items.map(order => <tr key={order.id}><td><Link className="text-link" to={`/cloud/orders/${order.id}`}>{order.id.slice(0, 18)}<ArrowRight size={14} /></Link><span className="cell-meta">{order.spec.instanceName}</span></td><td>{providers[order.provider] || order.provider}</td><td>{order.spec.instanceType}<span className="cell-meta">{order.spec.region} · {order.spec.zone}</span></td><td>{order.hourlyAmount} {order.currency}<span className="cell-meta">按小时</span></td><td><span className={`order-status ${order.status}`}>{labels[order.status] || order.status}</span></td><td>{new Date(order.updatedAt).toLocaleString()}</td><td><div className="table-actions"><Link className="icon-button" to={`/cloud/orders/${order.id}`} aria-label={`查看订单 ${order.spec.instanceName}`} title="查看订单"><ExternalLink size={15} /></Link><button className="icon-button danger-ghost" type="button" disabled={deletion.isPending || !['awaiting_confirmation', 'failed', 'expired'].includes(order.status)} onClick={() => remove(order)} aria-label={`删除订单 ${order.spec.instanceName}`} title={['awaiting_confirmation', 'failed', 'expired'].includes(order.status) ? '删除订单' : '已提交或结果不明的订单不可删除'}><Trash2 size={15} /></button></div></td></tr>)}</tbody></table></div></section> : <EmptyState title="还没有云订单" action={<Link className="button primary" to="/cloud/market">打开云资源市场</Link>} />}</div>;
}
...
function OrderDetail({ id }: { id: string }) {
  ...
  const query = useQuery({
    queryKey: ['cloud-order', id],
    queryFn: () => api.order(id),
    initialData: stateOrder,
    refetchInterval: current => current.state.data ? 10_000 : 1_000,
    ...orderQueryRecovery,
  });
  const order = query.data;
  const eventsQuery = useQuery({
    queryKey: ['cloud-order-events', id],
    queryFn: () => api.orderEvents(id),
    refetchInterval: 10_000,
    ...orderQueryRecovery,
  });
  const targetsQuery = useQuery({
    queryKey: ['cloud-order-target', id],
    queryFn: () => api.targets(true),
    enabled: Boolean(order && ['submitted', 'succeeded'].includes(order.status)),
    refetchInterval: 10_000,
    ...orderQueryRecovery,
  });
  const [sshTarget, setSshTarget] = useState<Target | null>(null);
  const provisionedTarget = targetsQuery.data?.items.find(target => target.orderId === id);
  const reconciliationContext = useQuery({
    queryKey: ['cloud-order-reconciliation', id],
    queryFn: () => api.orderReconciliationContext(id),
