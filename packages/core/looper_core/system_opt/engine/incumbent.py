...
- 🔴 作用域隔离（SO-D018，2026-08-23 审查 C3）：一个 tracker 实例只服务
  「同一组件的同一主指标」，跨组件/跨指标的效用不得进同一 incumbent 比较
  （S0 可比性）。引擎层负责按组件各持一个实例；"轮级整体效用"的跨组件
  聚合方案仍未定义，属 open decision，不得在此默认。
"""
