...
退化级的显式占位（依赖 S8 结果向量，未实现前禁止假装执行）。
"""
...
ROLLBACK_SCHEMA = "looper.rollback-record/v1alpha1"
...
def require_regression_support() -> None:
    """Fail closed: regression-level rollback execution waits for S8."""

    raise NotImplementedError(REGRESSION_DEPENDENCY)
