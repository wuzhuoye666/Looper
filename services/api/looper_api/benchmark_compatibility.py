...
def single_node_contract(manifest: dict[str, Any]) -> dict[str, Any] | None:
    ...
    primary_id = infrastructure.get("primaryNodeGroup")
    scored_groups = [
        item
        for item in node_groups
        if item.get("includedInScore", item.get("role") != "load-generator")
    ]
    if primary_id:
        scored_groups = [item for item in node_groups if item.get("id") == primary_id]
    if len(scored_groups) != 1:
        return None
    node_group = scored_groups[0]
    count = node_group.get("count") or {}
    if any(count.get(key) != 1 for key in ("minimum", "default", "maximum")):
        return None
    auxiliary_groups = [item for item in node_groups if item is not node_group]
    if any(
        item.get("role") != "load-generator"
        or item.get("includedInScore", False)
        or any(
            (item.get("count") or {}).get(key) != 1
            for key in ("minimum", "default", "maximum")
        )
        for item in auxiliary_groups
    ):
        return None
    return node_group
...
def _target_architectures(target: TargetRecord) -> set[str]:
    measured = _normalize_architecture(
        _first_value(target, "architecture", "machine", "cpu_architecture")
    )
    values = {measured} if measured else {
        _normalize_architecture(item) for item in target.capabilities_json
    }
...
def _target_os_families(target: TargetRecord) -> set[str]:
    measured = _normalize_os(_first_value(target, "system", "os_name", "platform", "framework"))
    values = {measured} if measured else {
        _normalize_os(item) for item in target.capabilities_json
    }
