...
| 开销 A/B | ✅ `build_collection_overhead_evidence`（L4 合同，成对裸墙钟）；2026-08-24 **O2** 活体探测携带开销 A/B 证据（`dynamic_collection.py` 相邻 disabled→enabled 配对，f46bc16；无阈值无裁决） | O1 的开销 A/B 属 G 泳道 G3 进行中（常态层逐窗配对成本不可接受，会话首窗配对方向）；真实机器 O2 开销验收仍属后续实测 |
