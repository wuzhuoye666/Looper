     ...
     └─ pure contract implementation (D5-I1, accepted 0c69cdd + 87d776c)
         └─ runtime wiring design (D5-I2-D R3 frozen)
             └─ L1 observer/backend boundary/durable receipt (D5-I2-A accepted 16c1fdd)
                 └─ dynamic-loop wiring (D5-I2-B accepted 8e657e5)
                     └─ CLI attention/restart reconcile seam (D5-I2-C accepted 2065a77)
```
...
| D5-I2-B dynamic-loop 两阶段接线 | glm5.3 | 重建后确认 parent `9f3fa07`；实现 `8e657e5`；`system-optimizer-impl` | 依赖 D5-I2-A；`dynamic_adapters.py`、`dynamic_loop.py`、`phase_gate.py`、`intervention_receipt.py` 的只读 head 枚举、专属 v2 tests；不改 CLI/backend/rollback | `accepted-b`。新增独立 v1alpha2 gate/proposal/window/run 分派，prepare→gate→execute，按 candidate `apply_started` 计 intervention/risky，candidate/recovery receipt 双链及 plan/outcome/digest 关联；v1 模型语义不迁移。修复 Windows 长路径下 receipt 原子替换。B 新增 5、B+旧聚焦 81、System Optimizer 全量通过，Ruff/diff 通过；工作树被外部 agent 删除后从远端 `9f3fa07` 重建并重新核验，未跟踪测试产物未提交 |
| D5-I2-C CLI attention/restart reconcile | glm5.3 | parent `8e657e5`；实现 `2065a77`；`system-optimizer-impl` | 依赖 D5-I2-B；仅 `cli.py` 与 dynamic CLI tests；不改 core/backend/collection/replay/rollback | `accepted-c`，push 待台账提交后一并执行。gate/proposal 先按 schema 分派且拒绝混代；v2 注入 target-bound AttentionSink 和 durable store；启动扫描验证所有 receipt head，post-apply 非终态禁止新 lease/执行、不自动 replay、标 needs-attention 并绑定最后可信 receipt；既有 attention 不覆盖；v1 CLI 行为保留。CLI 16、A/B/C 聚焦 90、System Optimizer 620、全仓 953、Ruff packages+services+tests、py_compile、diff 通过；仅既有 `.pytest_cache` 权限 warning |
