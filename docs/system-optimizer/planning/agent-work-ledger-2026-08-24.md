...
| L7H-01 refuted hypothesis cache 合同 | GPT agent；glm5.3 验收 | GPT revision chain ending `623111c`；主线按最终净内容接入 `491c855` | docs-only；冻结 v1 单一业务复测来源、完整 identity、同 JSONL 分派、显式 retention | `integrated/push-pending`。R3 删除不可验证的 `refute_kind`，不把 proposal schema 当语义 identity；旧 candidate schema 冻结；`git show --check` 通过 |
| S4-01D v1 冻结与 S3 最小接口 | DeepSeek agent；glm5.3 验收 | DeepSeek R2 `24c6dda`（R1 parent `67b03de`）；主线按最终净内容接入 `7bafd4e` | docs-only；不改公式/scorer/loop/CLI | `integrated/push-pending`。选择零生产代码迁移：现四维、公式 v1alpha1、排序顺序冻结；S4-V2/E_m/P-D-A-Q-T 延后；S3 使用同形 symptom 输入且只替换 rank；`git show --check` 通过 |
| S3-01 O1 在线路由 | glm5.3 | 主线 `9f5c648`，依赖 `7bafd4e` | 新增 `online_routing.py` + tests；仅修 scorer 的 signed adverse aggregate | `integrated/push-pending`。O1 snapshots 绑定 target/environment/measurement/protocol/formula，经现有 S4 v1 和 L8 scorer 产确定性 rank；缺失/不可用/重复/未覆盖组件 fail-closed，不混文件 rank；内容寻址 routing evidence + fixed index |
| L7H-02 假设负缓存实现 | glm5.3 | 主线 `eb4422b`，依赖 `491c855` | `negative_cache`、新 bridge、v2 loop callback、专属 tests | `integrated/push-pending`。候选/假设两 schema 同 JSONL 严格分派；完整身份变化 miss；仅已执行、身份可比、样本充分的业务复测拒绝进入 cache；显式 retention 文件缺失时 CLI 禁止启用；原子发布后才更新内存 |
| M3-INT/PROFILE runnable demo | glm5.3 | 主线 `4f6a4a0`，依赖 `9f5c648` + `eb4422b` | dynamic session/CLI/scenario profile/demo/tests；不改 backend/receipt/safety | `integrated/push-pending`。v2 `dynamic-run` 接在线路由、L7 admission/refutation 和 routing evidence；晋升后产原始/通用 Profile 固定顺序双基线报告并绑定环境/workload/formula/run/promotion/candidate；`system-opt m3-demo --workspace <empty-dir>` 走同一生产 CLI 的完整 synthetic 切片；接受路径不伪造 cache，拒绝路径恰写一条。CLI/profile 22、System Optimizer 662/995、全仓 995 全绿；Ruff/py_compile/diff 最终门通过后随台账提交统一 push |
