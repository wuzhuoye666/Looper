...
| M2 通用标准压力调优 | 受控环境输出 best observed | policy、metric contract、bootstrap improvement、Pareto、周期基线、共享预算和停止条件；synthetic general demo；阿里云 ECS 存储 5 候选真实多轮；统一 phased pressure contract；CPU/Memory/Network-loopback 首次校准；NUMA 单节点 unavailable 证据 | 批准各组件稳定阈值；CPU/Memory/Network 合法候选真实闭环；双节点 NUMA；真实跨机网络；跨组件组合复验和通用 Profile；当前新增校准没有候选收益结论 |
