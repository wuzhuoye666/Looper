...
- 实现模块：统计 `packages/core/looper_core/system_opt/**/*.py`，排除两个
  `__init__.py`。当前为 **17 个实现模块**；此前 15 个是加入 M1
  `state_evidence.py` 与 M2 `pressure.py` 前的快照。
- 修复前测试：8 个测试文件，**49 个 `def test_` 测试函数 / pytest 收集
  50 个 case**。差额来自一个 parametrized 测试函数展开为两个 case。
- 当前口径：15 个 System Optimizer 测试文件，**90 个 `def test_` 测试函数 / pytest
  收集 93 个 case**；仓库全量 pytest 收集 **314 个 case**。本轮 System Optimizer
  93/93 通过；独立 basetemp 排除已知 cloud confirmation token flaky 后 313/313 通过。
  已知 flaky 单独顺序复跑 10 次为 9 pass / 1 fail；它没有因隔离 basetemp 消失，不能写成
  314/314，也不能沿用上一提交的 293 或 283 口径。
...
| M2 通用标准压力调优 | 受控环境输出 best observed | policy、metric contract、bootstrap improvement、Pareto、周期基线、共享预算和停止条件；synthetic general demo；阿里云 ECS 存储 5 候选真实多轮；统一 phased pressure contract；CPU/Memory/Network-loopback 首次校准与 target-local CV gate；Memory THP 两候选真实闭环且无候选晋级；**CPU governor 候选闭环完成**（tuned 停止交接、schedutil 新基线重校准、powersave 被稳定性门禁拒/performance LCB≤0 拒、uniform 单逻辑项建模、全部回滚）；**NUMA 双机单节点 unavailable 证据**（8C 与 24C 两台） | 双节点 NUMA（当前目标池不可得）；第二受控 peer 上的真实网络闭环（第二机于会话中途失联，loopback 证据已有）；组合复验与通用 Profile 机制未经验证（存储/内存/CPU 至今零个已接受候选，无可组合项，本目标通用结论为"保持默认配置"）；Memory/CPU 本轮均不能宣称优化收益 |
...
| M5 交付收尾 | 完整运行手册、实录、迁移和失败演练 | 文档树、公式溯源、synthetic/WSL2/阿里云单候选、多候选与组件校准证据；System Optimizer 93/93、仓库排除已知 flaky 后 313/313 | 无完整 replay/验证器、跨组件 failure drill、迁移说明、schema 稳定承诺和腾讯云 CVM 实录；已知 cloud token 真实时间 flaky 未修复 |
