...
- 当前口径：9 个 System Optimizer 测试文件，**61 个 `def test_` 测试函数**；仓库全量
  pytest 收集 **283 个 case**。本地独立 basetemp 与阿里云 ECS 完整依赖部署后的独立
  basetemp 均全部通过。
...
|---|---|---|---|
| M1 配置资产与安全底座 | 可安全采集、人工修改和恢复 | manifest/profile；显式根 raw inventory；环境指纹；动态域证据；simulated/local-linux/SSH backend；argv allowlist runner；preflight、snapshot、verify、rollback、租约；CLI validate/inventory/raw/manual；阿里云 ECS scheduler/nomerges 真实写入与恢复 | 腾讯云 CVM 直接实测；20 项逐项目标验证；持久化与 ownership 自动发现；崩溃对账和真实 rollback failure 演练 |
