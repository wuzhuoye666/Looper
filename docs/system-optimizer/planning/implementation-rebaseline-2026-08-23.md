...
- 当前口径：10 个 System Optimizer 测试文件，**69 个 `def test_` 测试函数 / pytest
  收集 70 个 case**；仓库全量 pytest 收集 **291 个 case**。本轮 M1 定向 25/25
  通过；独立 basetemp 排除已知 cloud confirmation token flaky 后 290/290 通过。
  未排除全量在该已知测试失败，不能写成 291/291，也不能沿用上一提交的 283/283。
...
|---|---|---|---|
| M1 配置资产与安全底座 | 可安全采集、人工修改和恢复 | manifest/profile；显式根 raw inventory；环境指纹；动态域证据；simulated/local-linux/SSH backend；argv allowlist runner；preflight、snapshot、verify、rollback、租约；逐项来源/持久化/ownership 状态证据；显式 actor 授权；状态证据绑定运行结果；完整快照崩溃对账和 attention 恢复 CLI；阿里云 ECS scheduler/nomerges 真实写入与恢复 | 腾讯云 CVM 直接实测；20 项逐项目标验证；跨 sysctl.d/tuned/发行版的自动优先级推断；真实 crash/rollback failure 演练；本轮 291 case 全量回归 |
